...
class MetaAgent(object):
    def __init__(self, env : RoboticsObservationWrapper, goal_list = ['microwave'], replay_buffer_size=1000000, max_episode_steps=500):
        self.agent_dict = {}
    ...
    def initilise_memory(self, augment_rewards=True, augment_data=True, augment_noise_ratio=0.1):
    ...
    def initilise_agents(self, gamma=0.99, tau=0.005, alpha=0.1,
                         target_update_intervall=2, hidden_size=512,
                         learning_rate=0.0001):
        for goal in self.goal_dict:
            self.env.set_goal(self.goal_dict[goal])
            obsevation, info = self.env.reset()
            obsevation_size = obsevation.shape[0]

            agent = Agent(obsevation_size, self.env.action_space, gamma=gamma,
                          tau=tau, alpha=alpha, target_update_interval=target_update_intervall,
                          hidden_size=hidden_size, learning_rate=learning_rate, goal=goal)
    ...
    def test(self):
        ...
        for goal in self.goal_dict:
            print(f"Attemting goal {goal}...")
            self.env.set_goal(self.goal_dict[goal])
            self.agent = self.agent_dict[goal]


            action, reward = self.agent.test(env=self.env, episodes=1, max_episode_steps=self.max_episode_steps,
                                             prev_action=action)
